...
因此，首个交付不建设面向低技术用户的管理页面，而是先建立稳定、可复用的内部只读 API。Thymeleaf 管理页、认证授权和更丰富的交互将在后续阶段基于同一 API 建设。
...
| API 形态 | 单一 `GET /api/v1/corpus` | 只读查询参数扁平，便于浏览器、Swagger、脚本和未来管理页复用。 |
...
本阶段明确未实现：Thymeleaf 管理页、认证与角色权限、多关键词布尔逻辑、多标签组合、原评论发布时间筛选、标签建议接口、MySQL Full-Text、Elasticsearch 和游标分页。
